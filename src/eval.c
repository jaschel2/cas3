/* eval.c ── recursive-descent expression evaluator
   Variables: x y z  (also aliased as u v / r theta / rho phi theta)
   Constants: pi  e
   Ops:       + - * / ^  (^ is right-associative)  unary -
   Functions: sin cos tan asin acos atan sqrt ln log exp abs
   Shorthand: p  → pi  (F3 key shortcut on the calc)
*/
#include "cas.h"
#include <ctype.h>
#include <stdlib.h>
...
static double p_pow(EC*c){
    double v=p_unary(c);
    if(!c->err && eat(c,'^')) v=pow(v,p_pow(c)); /* right-assoc */
    return v;
}
...
static double p_primary(EC*c){
    ...
    if(isalpha(ch)||ch=='_'){
        ...
        /* variables / aliases */
        if(!strcmp(nm,"x")||!strcmp(nm,"u")||!strcmp(nm,"r")||!strcmp(nm,"rho")) return c->vx;
        if(!strcmp(nm,"y")||!strcmp(nm,"v")||!strcmp(nm,"theta")||!strcmp(nm,"phi")) return c->vy;
        ...
        /* bare 'e' without () already handled above */
        c->err=1; return 0;
    }
