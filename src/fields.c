/* fields.c ── define the input fields for every operation */
#include "cas.h"

/* helper: set one field */
...
void fields_load(Op op, Coords coords){
    ...
    /* ── 2. SURFACE AREA ─────────────────────────────────────────
       r(u,v) = <x,y,z>,  u in [u0,u1],  v in [v0,v1]           */
    case OP_SURFAREA:
        sf(0,"x(u,v)",    "u+2*v");
        sf(1,"y(u,v)",    "6*v");
        sf(2,"z(u,v)",    "u-v");
    ...
    /* ── 4. FLUX ─────────────────────────────────────────────────
       ∬ F·dS  (orientation set on previous screen)              */
    case OP_FLUX:
        sf(0,"x(u,v)",    "u");
        sf(1,"y(u,v)",    "v");
        sf(2,"z(u,v)",    "v*u");
    ...
    /* ── 5. VOLUME between two surfaces (polar) ──────────────────
       ∬ (z_top - z_bot) · r dr dθ                               */
    case OP_VOLUME:
        sf(0,"z top(r)",  "4-r^2");
        sf(1,"z bot(r)",  "3*r^2");
    ...
    /* ── 6. TRIPLE INTEGRAL ──────────────────────────────────────
       ∭ f dV — labels change with coordinate system             */
    case OP_TRIPLE:
        sf(0,"f(x,y,z)",  "x*z");
        switch(coords){
        case COORD_CART:
...
/* copy defaults into current values */
