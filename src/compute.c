...
static void do_curldiv(void){
    ...
    double ci=Ry-Qz, cj=Pz-Rx, ck=Qx-Py;
    double dv=Px+Qy+Rz;

    char bi[16],bj[16],bk[16],bd[16],bm[16];
    fmt(bi,16,ci); fmt(bj,16,cj); fmt(bk,16,ck);
    fmt(bd,16,dv);
    fmt(bm,16,sqrt(ci*ci+cj*cj+ck*ck));
...
static double sa_f(double u,double v,void*p){
    ...
    surf_partials(s,u,v,&rux,&ruy,&ruz,&rvx,&rvy,&rvz);
    return cross_mag(rux,ruy,ruz,rvx,rvy,rvz);
}
static void do_surfarea(void){
    SACtx ctx; ctx.s.xs=G.f[0].val; ctx.s.ys=G.f[1].val; ctx.s.zs=G.f[2].val;
    ...
    char b[20],bp[24]; fmt(b,20,r);
    fmt(bp,20,r/M_PI); char tmp[28]; snprintf(tmp,28,"%s*pi",bp);
...
/* ══════════════════════════════════════════════════════════════════════════
   5. VOLUME between two surfaces (polar coords)
   ══════════════════════════════════════════════════════════════════════════ */
typedef struct{ const char*zt,*zb; } VOLCtx;
static double vol_f(double r,double th,void*p){
    VOLCtx*c=(VOLCtx*)p;
    /* x=r*cos(th), y=r*sin(th), z unused — surface given as f(r) only */
    double top=eval(c->zt,r,th,0);
    double bot=eval(c->zb,r,th,0);
    double h=top-bot; if(h<0)h=0;
    return h*r;   /* polar Jacobian */
}
...
static double ti_f(double a,double b,double c_,void*p){
    ...
    case COORD_CYL:  /* a=r, b=theta, c=z */
        x=a*cos(b);y=a*sin(b);z=c_;jac=a;break;
    default: /* SPHERICAL  a=rho, b=phi, c=theta */
        x=a*sin(b)*cos(c_);
        y=a*sin(b)*sin(c_);
        z=a*cos(b);
        jac=a*a*sin(b);
